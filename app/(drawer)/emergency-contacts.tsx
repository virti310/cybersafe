...
export default function EmergencyContacts() {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                const sortedData = [...data].sort((a, b) => {
                    const isACyber = a.team?.toLowerCase().includes('cyber crime helpline');
                    const isBCyber = b.team?.toLowerCase().includes('cyber crime helpline');
                    if (isACyber && !isBCyber) return -1;
                    if (!isACyber && isBCyber) return 1;
                    return a.id - b.id;
                });
    ...
    const getPriorityColor = (priority: string) => {
        switch (priority?.toLowerCase()) {
            case 'cyber fraud': return '#EF5350';     // Red for Fraud
            case 'police & safety': return '#5C6BC0'; // Indigo for Police
            case 'banking': return '#42A5F5';         // Blue for Banking
            case 'citizen help': return '#AB47BC';    // Purple for Citizens (changed from Teal to avoid clashing with Call button)
            default: return '#90A4AE';                // Default Grey
        }
    };
